...
PROFILE_SYSTEM = (
    "You are an OCR system for handwritten Ukrainian guest cards from women's networking events. "
    "The cards are filled by young women (20-35) in Kyiv at social/networking evenings. "
    "Handwriting is cursive Ukrainian — letters connect and are hard to separate. "
    "When uncertain, provide your BEST GUESS as the first option, then alternatives. "
...
4. Для "low" або "medium" confidence — ОБОВ'ЯЗКОВО додай поле в unclear_fields з:
   - "raw_chars": ОБОВ'ЯЗКОВО звичайний текст словами злитно!
     ✅ ПРАВИЛЬНО: "ілюстративний фріланс, ходжу писати картини"
     ❌ ЗАБОРОНЕНО: "і л ю с т р а т и в н о м у" (окремі літери з пробілами)
     ❌ ЗАБОРОНЕНО: "і-л-ю-с-т-р-а-т-и-в-н-о-м-у" (літери з дефісами)
     ❌ ЗАБОРОНЕНО: "і | л | ю | с | т | р" (літери з розділювачами)
     Пиши СЛОВА як в звичайному реченні!
   - "possible_readings": РІВНО 3 варіанти — від найбільш до найменш ймовірного.
     Використовуй контекст (типові професії, хобі, риси) щоб підібрати слова що збігаються з літерами.
     Якщо жоден варіант не підходить — третій може бути побуквенний запис як є.

5. КОНТЕКСТ — це анкети молодих жінок 20-35 років на нетворкінг-вечорах у Києві.
   Типові відповіді в полях (використовуй як підказку, але ТІЛЬКИ якщо літери збігаються):

   Чим займаюсь (професії): маркетолог, дизайнер, фотограф, стиліст, PR, SMM, IT, HR, юрист,
   психолог, косметолог, візажист, нерухомість, фінанси, підприємиця, фрілансер, менеджер,
   вчителька, лікар, продюсер, блогер, нутріціолог, тренер, бариста, флорист, event-менеджер

   Хобі: йога, пілатес, танці, малювання, спорт, подорожі, фото, таро, готування, читання,
   вишивання, в'язання, серфінг, сап, біг, плавання, музика, спів, гітара, кіно, театр,
   волонтерство, медитація, керамiка, скалолазіння, бокс, стрільба, верхова їзда, садівництво

   Краща/найгірша риса: відповідальна, пунктуальна, емпатична, наполеглива, перфекціоністка,
   впертість, лінь, емоційність, довірливість, прямолінійність, надмірна відповідальність,
   нетерплячість, контрол-фрік, занадто добра, самокритичність, обдумую рішення довго

6. Для чекбоксів: будь-яка позначка = true, порожній = false
7. Порожнє поле = null
8. Якщо бачиш "/" або "," між словами — це СПИСОК (декілька хобі/рис через роздільник)
...
3. Склади слово зі знайдених літер
4. Запиши результат ЗВИЧАЙНИМ ТЕКСТОМ (без дефісів між літерами)
5. Якщо слово не складається в відоме українське слово — запиши як є
...
- letters: побуквенний розбір звичайним текстом
...
Дозволені коди: П (подруга), К (колаборація), Б (бізнес), Т (travel), + (хочу спілкуватись), ? (свій варіант).
